...
export default async function AdminLojaTicketsArquivoPage({
  searchParams,
}: {
  ...
  const { tickets, total } = await listarArquivoTickets(tenant.id, {
    filtro,
  ...
  function hrefFiltro(f: ArquivoTicketsFiltro) {
    ...
    return `/admin/loja/tickets?${sp.toString()}`
  }

...
export function AdicionarSacolaForm({ produto }: { produto: Produto }) {
  ...
  if (state.success) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center dark:border-emerald-800 dark:bg-emerald-950">
        ...
        <h3 className="font-semibold text-emerald-800 dark:text-emerald-200">Adicionado à sacola!</h3>
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      {state.error && (
        <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-950 dark:text-red-400">{state.error}</p>
      )}

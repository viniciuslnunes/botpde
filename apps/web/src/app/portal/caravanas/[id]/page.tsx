...
export const metadata: Metadata = { title: 'Caravana' }

type Params = { id: string }

export default async function PortalCaravanaDetailPage({
  params,
}: {
  ...
  const evento = await getEventoEmbarque(tenant.id, id, 'CARAVANA')
  if (!evento) notFound()
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        {passado && (
          <p className="mt-1 text-xs font-medium text-[rgb(var(--foreground-muted))]">Encerrada</p>
        )}
      </div>

      <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-5 text-sm space-y-2">
        ...
        {evento.descricao && (
          <p className="pt-2 text-[rgb(var(--foreground-muted))] whitespace-pre-wrap">
            {evento.descricao}
          </p>
        )}
      </div>

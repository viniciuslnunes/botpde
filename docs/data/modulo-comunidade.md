...
| Thread | `Comentario.parentId` aponta para o comentário respondido (não só a raiz). Qualquer nível aceita várias respostas. A UI achata os descendentes num bloco sob a raiz (`achatarRespostasDaArvore` + barra “N respostas” no padrão Meu Timão) — sem escada de indentação. Quem recebe a resposta é notificado (`NOVO_COMENTARIO`, título “Resposta no seu comentário”). |
| Editar/excluir comentário | `editarComentario` / `excluirComentario` — `findFirst` por `autorId` da sessão. Só o autor; moderação oculta, não reescreve. UI única: `ComentarioMenu` (`components/portal/comentario-menu.tsx`), reusado por post, reel, feed e detalhe de notícia/fórum via `editarAction`/`excluirAction`. Ao excluir, quem guarda a lista em estado reparenta os órfãos (`parentId → null`) — igual a `montarArvoreComentarios`, senão a resposta some da tela sem ter sido apagada. |

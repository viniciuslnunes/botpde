...
.DESCRIPTION
  ...
  A URL de origem é lida de apps/web/.env.local (DATABASE_URL) ou, se você já
  tiver trocado essa variável para o banco local, de DATABASE_URL_RAILWAY no
  mesmo arquivo. Nenhuma credencial fica no script.
...
# Origem: preferimos DATABASE_URL_RAILWAY (existe depois que .env.local já
# aponta para o local); caímos em DATABASE_URL enquanto ela ainda for a remota.
$sourceUrl = Read-EnvVar -Path $EnvFile -Name 'DATABASE_URL_RAILWAY'
if (-not $sourceUrl) { $sourceUrl = Read-EnvVar -Path $EnvFile -Name 'DATABASE_URL' }
if (-not $sourceUrl) { throw "Não achei DATABASE_URL nem DATABASE_URL_RAILWAY em $EnvFile" }
...
Write-Host ''
Write-Host 'Pronto. Aponte apps/web/.env.local para o banco local:' -ForegroundColor Green

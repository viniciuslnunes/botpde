  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (interaction.isButton() && interaction.customId === 'prod_sem_foto') {
        ...
        const logsItensLojaCh0 = await interaction.client.channels.fetch(config.canais.logsItensLoja).catch(() => null);
        if (logsItensLojaCh0) {
          logsItensLojaCh0.send({ embeds: [{ color: 0x000000,
        ...
        const msg0 = await interaction.fetchReply().catch(() => null);
        if (msg0) setTimeout(() => msg0.delete().catch(() => {}), 60_000);
        return;
      }
      ...
      if (interaction.isButton() && interaction.customId === 'prod_manter_fotos') {
        ...
        const logsItensLojaCh1 = await interaction.client.channels.fetch(config.canais.logsItensLoja).catch(() => null);
        if (logsItensLojaCh1) {
          ...
          logsItensLojaCh1.send({ ...carrosselEmbeds(embedLog, p.imagem_url) });
        }
        await interaction.update({
          embeds: [{ color: 0x000000,
        ...
        if (msg1) setTimeout(() => msg1.delete().catch(() => {}), 60_000);
        return;
      }
